...
MeetingTruth 是一个 macOS 端会议事实复核应用。它把多路 ASR 候选、会议材料、截图/白板/手写图片、OCR 线索和人工确认组织成证据链，再由 Gemma 4 进行冲突发现、native function calling、多模态证据融合和最终成果生成。

核心目标不是“把音频总结得更流畅”，而是在纪要生成前降低事实错误。系统重点处理人名、金额、时间、项目名、系统名、承诺事项和专业术语这类高风险事实。
